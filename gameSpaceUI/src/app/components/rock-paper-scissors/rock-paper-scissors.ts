...
export class RockPaperScissors implements OnInit {
  ...
  // Image Paths
  // Assumes you have these in your assets folder
  ...
  play(playerMove: string) {
    // 1. STOP any existing confetti immediately
    this.stopConfetti();
  ...
  handleWin() {
    ...
    this.score.wins++;
    // Neon Green Background with transparency
  ...
  handleLoss() {
    ...
    this.score.losses++;
    // Neon Red Background with transparency
  ...
  handleTie() {
    ...
    this.score.ties++;
    // Neon Yellow Background with transparency
  ...
  triggerConfetti() {
    ...
    // We use an arrow function here to keep 'this' context
    const frame = () => {
      // If flag is turned off (by next click), stop the loop
      ...
      confetti({ 
        ...
        spread: 55, 
        origin: { x: 0 },
        colors: ['#6366f1', '#ec4899', '#eab308'] // Custom Neon Colors
      });
      confetti({ 
        particleCount: 5, 
        angle: 120, 
  ...
  stopConfetti() {
    // 1. Sets flag to false, which breaks the requestAnimationFrame loop above
    this.isConfettiActive = false; 
    // 2. Immediately clears canvas of existing particles
    confetti.reset(); 
  }
}
